...
def candidate_from_row(row: dict[str, Any]) -> dict[str, Any]:
    candidate = {
        ...
        "trigger_scope_node_count": int(float(row.get("trigger_scope_node_count", row.get("trigger_node_count", 5)))),
...
def evaluate_attack_candidate(
    *,
    ...
    poisoned_train = build_poisoned_training_set(
        ...
        target_region_loss_weight=float(candidate.get("target_region_loss_weight", config["poison"].get("target_region_loss_weight", 1.0))),
        trigger_scope_node_count=int(candidate.get("trigger_scope_node_count", candidate["trigger_node_count"])),
